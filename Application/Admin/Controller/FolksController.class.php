...
class FolksController extends CommonController {
    public function index() {
            ...
            $Folkss =D('Staff')->select($data,$page,$pageSize);
           $menusCount =count($Folkss);
            $this->assign('Count', $menusCount);
           $this->assign('Folkss', $Folkss);
    ...
    public function edit() {
        $id=$_GET['id'];
        $Folks = D("Staff")->getAdminByStaffId($id);
        $model['model']=1;
        $departs = D("Depart")->getNormalDeparts($model);
        $this->assign('departs', $departs);
        $this->assign('Folks',$Folks);
        $this->display();
    }
