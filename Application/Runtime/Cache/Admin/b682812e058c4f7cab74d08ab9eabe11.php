...
<div class="page-container">
        <div class="row">
          <form action="./admin.php" method="get">
			...
			<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="admin_add('添加管理员',SCOPE.add_url,'800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加管理员</a></span> <span class="r">共有数据：<strong><?php echo ($Count); ?></strong> 条</span>
			</div>
		...
		<tbody>
		 <?php if(is_array($admins)): $i = 0; $__LIST__ = $admins;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				<td><input type="checkbox" value="<?php echo ($vo["admin_id"]); ?>" name="listorder[<?php echo ($vo["admin_id"]); ?>]"></td>
				<td><?php echo ($vo["admin_id"]); ?></td>
				<td><?php echo ($vo["username"]); ?></td>
				<td><?php echo ($vo["realname"]); ?></td>
				<td><?php echo ($vo["email"]); ?></td>
				<td><?php echo (date("Y-m-d H:i",$vo["createtime"])); ?></td>
				<?php if($vo["status"] == -1): ?><td class="td-status" attr-id="<?php echo ($vo["admin_id"]); ?>" attr-status="<?php echo ($vo["status"]); ?>"><span class="label label-success radius" >未启用</span></td>
				<?php else: ?>
				  <td class="td-status" attr-id="<?php echo ($vo["admin_id"]); ?>" attr-status="<?php echo ($vo["status"]); ?>"><a style="text-decoration:none" onClick="admin_stop(this,'10001')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a></td><?php endif; ?>
				<td>
		          <a title="删除" attr-id="<?php echo ($vo["admin_id"]); ?>" id="smart-delete" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
